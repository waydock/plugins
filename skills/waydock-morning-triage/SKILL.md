...
Do not call `waydock_draft_reply_save` yet. It writes a real draft into the
user's mailbox, and a draft they did not ask for is a surprise rather than a
service. Waydock's own `draft_reply` prompt says the same thing: do not save
without explicit confirmation. Once the user says yes, save the ones they picked.
